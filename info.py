...
func_for_empty_scopes = r'(?:(?:d(?!iv))?(?:arc)?(?:sin|cos|c?tg)|d?arc|lg|ln|d)'
...
letters_and_inwords, letters_and_not_example_like_inwords, syntax = r'[а-яА-ЯЁё"\'«»“”()-]', r'[а-яА-ЯЁё"\'«»“”]', r'(?:[.,!?;:/—–]|\s[—–]|\s-\s(?=\w|\d+-\w))'
...
text_help_calc = ("  строка-калькулятор | быстрый ввод • мгновенное вычисление • точный ответ\n\n"
                  ...
                  " .1. Скобки: () [[] или o] (скобки – rOund brackets); модуль: | [\\] [a] (Absolute)\n"
                  " .2. Факториал от числа x: x! [f] (Factorial)\n"
                  " .3. Корень: √ [r] (Root | Radical), N√M=N^(1/M), не путать с N•√M\n"
                  " .4. Логарифмы: logAbyB [l] (Log), ln [n] (Natural log), lg [g] (General log), by [b]. Тригонометрия: sin [s]; cos [c]; tg [t]; ctg [C]\n"
                  " .5. Степень: ^ [w] poWer [xx]\n"
                  ...
                  " ● [esc] ESCape, ctrl+w закрывают окно в фокусе приложения, а левая кнопка мыши открывает/закрывает окно истории\n"
                  " ● [`] (где [ё]) очищает поле ввода, ['] копирует пример в буфер обмена, [\"] копирует ответ, [enter] заменяет пример в поле ввода на ответ\n"
                  " ● двойным нажатием можно выделить пример целиком\n"
                  ...
                  " ● ctrl+shift+z перемещает на один пример в прошлое, ctrl+shift+y — в будущее, прокрутка колеса в поле ввода прокручивает историю с подсветкой\n"
                  " ● ctrl+z перемещает на 1 действие назад, ctrl+y — вперёд, также делает shift+скролл вверх-низ, нажатия/скроллы по ленте действий крутят быстрее\n"
                  ...
                  " ⬥ [/] — '1/' (после '√•^' — '2/'), [+] после '•/^√' введётся с '2', иначе заменится на '1'; после '.' ставь любой знак, кроме [.] — появится '5'\n"
                  ...
                  " ★ Автоматическое добавление необходимых знаков при вводе примера, работает с примером с пробелами так, как если б их не было\n"
                  ...
                  " ★ При вставке табличных данные (даже с пустыми ячейками) разделяются плюсами, так будет посчитана сумма данных\n"
                  " ★ Открой строку за 15 секунд после закрытия и пример останется. Иначе перейдёт в историю, если в ответе не число — удалится\n"
                  "\nНастройки и вид\n"
                  " ⚙ История. окно истории можно смахнуть вбок по тачпаду, либо утащить вбок за бегунок на полосе прокрутки\n"
                  " ⚙ Размещение. Перетаскивай строку мышкой (если тащищь в границу, она свернётся), через ctrl+↑ или написав 'y#', '-y#' (y — отступ сверху, -y снизу)\n"
                  ...
                  " ⚙ Увеличивай и уменьшай текст комбинациями [ctrl]+[±] или сведением/разведением двух пальцев на тачпаде\n"
                  " ⚙ Разворачивай/сворачивай через ctrl+↓. Строка свернётся когда прокрутке колеса листать историю некуда, развернётся при обратном движении\n"
                  "    Также можно сворачивать строку при перетаскивании окна к стенке в то время, когда окно уже на границе\n"
                  ...
                  " ❗ Знак умножить обязателен: нельзя 5sin90, можно 5•sin90")
